...
// -----------------------------------------------------------------------
// Constants for this specialised page
// -----------------------------------------------------------------------
const ALLOWED_SERVICOS = ['Medicina Geral', 'Enfermagem'];
const FIXED_LOCAL = 'Casa de Saúde';

// Medicina Geral: Monday (1) and Thursday (4) only.
// The HTML date input value is yyyy-MM-dd. We derive the weekday from that.
const isAllowedForMedicinaGeral = (dateStr: string): boolean => {
  if (!dateStr) return true; // no date yet — don't block
...
export default function ConsultasCasaSaudePage() {
  const { role } = useAuth();
  ...
  // Date validation error for Medicina Geral weekday restriction
  ...
  // ------------------------------------------------------------------
  // Fetch — only Medicina Geral and Enfermagem at Casa de Saúde
  // ------------------------------------------------------------------
  const fetchData = async () => {
    ...
    if (consultasRes.error) {
      ...
      // Filter in JS as well to only show allowed services (belt-and-suspenders)
      const rawRows = ((consultasRes.data ?? []) as any[]).filter(
        (c) => ALLOWED_SERVICOS.includes(c.servicos?.nome ?? '')
      );
  ...
  // ------------------------------------------------------------------
  // Helpers: determine selected service name
  // ------------------------------------------------------------------
  const selectedServicoNome = (() => {
    const s = servicos.find((sv) => sv.id === formData.servico_id);
    return s?.nome ?? '';
  })();

  const isMedicinaGeral = selectedServicoNome === 'Medicina Geral';
  ...
  // ------------------------------------------------------------------
  // Date change — validate weekday for Medicina Geral
  // ------------------------------------------------------------------
  const handleDateChange = (dateStr: string) => {
    setFormData({ ...formData, data: dateStr });
    if (isMedicinaGeral && dateStr && !isAllowedForMedicinaGeral(dateStr)) {
      setDateError('Medicina Geral: apenas segunda-feira e quinta-feira');
    } else {
      setDateError('');
    }
  };

  // Also validate when service changes
  const handleServicoChange = (value: string) => {
    setFormData({ ...formData, servico_id: value });
    const nome = servicos.find((s) => s.id === value)?.nome ?? '';
    if (nome === 'Medicina Geral' && formData.data && !isAllowedForMedicinaGeral(formData.data)) {
      setDateError('Medicina Geral: apenas segunda-feira e quinta-feira');
    } else {
      setDateError('');
    }
  ...
  const openEditModal = (consulta: ConsultaRow) => {
    ...
    setFormData({
      ...
      notas: '',
      servico_id: '',
      local: consulta.local ?? FIXED_LOCAL,
    });
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    // Weekday guard for Medicina Geral
    if (!editingConsulta && isMedicinaGeral && !isAllowedForMedicinaGeral(formData.data)) {
      toast.error('Medicina Geral: selecione uma segunda-feira ou quinta-feira');
      return;
    }
  ...
  const handleDelete = async () => {
    ...
    if (error) {
      if (error.code === '42501') {
        toast.error('Não tem permissão para eliminar esta consulta.');
      } else {
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Consultas Casa de Saúde');
    XLSX.writeFile(wb, `consultas_casa_saude_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  return (
    <div className="page-enter flex flex-col h-full gap-3">
      {/* BLOCO 1: PageHeader */}
      <PageHeader title="Casa de Saúde" description="Medicina Geral e Enfermagem">
        {canEdit && (
      ...
      <div className="border border-border/50 rounded-lg overflow-hidden flex-1 flex flex-col">
        <div className="flex-1 overflow-auto">
          <DataTable
            ...
            emptyDescription="Ainda não existem consultas registadas para Casa de Saúde."
          ...
          <div className="max-h-[90vh] overflow-y-auto px-6 py-6">
            <DialogHeader className="mb-6">
              <DialogTitle className="flex items-center gap-2 text-lg font-semibold">
                ...
                {isEditing ? 'Editar Consulta' : 'Nova Marcação — Casa de Saúde'}
              </DialogTitle>
            ...
            <div className="space-y-2" ref={nifContainerRef}>
              <Label className="text-sm font-medium text-foreground">NIF do Paciente *</Label>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* Local é fixo — campo informativo, não editável */}
              <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label className="text-sm font-medium text-foreground">
                  Data *
                  {isMedicinaGeral && (
                    <span className="ml-1 text-xs text-muted-foreground">(2ª ou 5ª feira)</span>
                  )}
                </Label>
                <DatePickerInput
                  value={formData.data}
